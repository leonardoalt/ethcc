...
contract ERC777 is Context, IERC777, IERC20 {
	...
	bool public totalSupplyChanged;
    ...
    function _mint(
        ...
        _callTokensReceived(operator, address(0), account, amount);

        emit Minted(operator, account, amount);
    ...
    function _send(
        ...
        _move(operator, from, to, amount);

        _callTokensReceived(operator, from, to, amount);
    }
    ...
    function _burn(
        ...
        _callTokensToSend(operator, from, address(0), amount);
        ...
        emit Burned(operator, from, amount);
    ...
    function _move(
        ...
        emit Sent(operator, from, to, amount);
        emit Transfer(from, to, amount);
    }
	
	function allowance(address owner, address spender) public view override returns (uint256) { return 0; }
	function approve(address spender, uint256 amount) public override returns (bool) { return true; }
    function transferFrom(
        address sender,
        address recipient,
        uint256 amount
    ) public override returns (bool) { return true; }
